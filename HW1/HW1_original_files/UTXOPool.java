...
public class UTXOPool {
    ...
    /** Creates a new empty com.ninjoncrack.crypto.hw1.UTXOPool */
    public UTXOPool() {
        H = new HashMap<UTXO, Transaction.Output>();
    }

    /** Creates a new com.ninjoncrack.crypto.hw1.UTXOPool that is a copy of {@code uPool} */
    public UTXOPool(UTXOPool uPool) {
        H = new HashMap<UTXO, Transaction.Output>(uPool.H);
    }

    /** Adds a mapping from com.ninjoncrack.crypto.hw1.UTXO {@code utxo} to transaction output @code{txOut} to the pool */
    public void addUTXO(UTXO utxo, Transaction.Output txOut) {
        H.put(utxo, txOut);
    }

    /** Removes the com.ninjoncrack.crypto.hw1.UTXO {@code utxo} from the pool */
    public void removeUTXO(UTXO utxo) {
        H.remove(utxo);
    }

    /**
     * @return the transaction output corresponding to com.ninjoncrack.crypto.hw1.UTXO {@code utxo}, or null if {@code utxo} is
     *         not in the pool.
     */
    ...
    /** @return true if com.ninjoncrack.crypto.hw1.UTXO {@code utxo} is in the pool and false otherwise */
